...
const LoanOption = (props) => {
  ...
  const fetchLoanOptions = async () => {
    try {
      const response = await ConstantService.fetchAllLoanOption();
  ...
  const onSubmitClickListener = async values => {
    try {
      const response = await ConstantService.createLoanOption(values);
    ...
    <div className={classes.root}>
      <Box py={3} px={5}>
        <Grid container justifyContent="space-between">
          <Typography variant="h5">Manage Loan Option</Typography>
          <Button
            variant="contained"
      ...
      <Paper style={{ marginLeft: 20, marginRight: 20, borderRadius: 5 }}>
        ...
        <TableBody>
          {loanOptions &&
            loanOptions.slice(0, limit).map((item, i) => (

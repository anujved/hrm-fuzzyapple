...
const PayslipType = (props) => {
  ...
  const fetchPayslipTypes = async () => {
    try {
      const response = await ConstantService.fetchAllPayslipType();
      setPayslip(response);
    } catch (error) {}
  }
  ...
  const onSubmitClickListener = async values => {
    try {
      const response = await ConstantService.createPayslipType(values);
    ...
    <div className={classes.root}>
      <Box py={3} px={5}>
        <Grid container justifyContent="space-between">
          <Typography variant="h5">Manage Payslip Type</Typography>
          <Button
            variant="contained"

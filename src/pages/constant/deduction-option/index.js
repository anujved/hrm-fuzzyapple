...
const DeductionOption = (props) => {
  ...
  const fetchDeductionOptions = async () => {
    try {
      const response = await ConstantService.fetchAllDeductionOption();
      setDeductionOptions(response);
    } catch (error) {}
  }
  ...
  const onSubmitClickListener = async values => {
    try {
      const response = await ConstantService.createDeductionOption(values);
    ...
    <div className={classes.root}>
      <Box py={3} px={5}>
        <Grid container justifyContent="space-between">
          <Typography variant="h5">Manage Deduction Option</Typography>
          <Button
            variant="contained"
      ...
      <Paper style={{ marginLeft: 20, marginRight: 20, borderRadius: 5 }}>
        ...
        <TableHead>
          <TableRow>
            <TableCell>Loan Option</TableCell>
            <TableCell align="right">Action</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {deductionOptions &&
            deductionOptions.slice(0, limit).map((item, i) => (
